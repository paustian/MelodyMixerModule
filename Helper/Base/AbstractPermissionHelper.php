...
abstract class AbstractPermissionHelper
{
    ...
    public function hasEntityPermission(EntityAccess $entity, int $permissionLevel, int $userId = null): bool
    {
        ...
        return $this->permissionApi->hasPermission(
            'PaustianMelodyMixerModule:' . ucfirst($objectType) . ':',
            $instance,
            $permissionLevel,
            $userId
        );
    ...
    {
        $filteredEntities = [];
        foreach ($entities as $melodyMixer) {
            if (!$this->hasEntityPermission($melodyMixer, $permissionLevel, $userId)) {
                continue;
            }
            $filteredEntities[] = $melodyMixer;
        }
