...
abstract class AbstractScoreController extends AbstractController
{
    ...
    protected function viewInternal(
        ...
        $routeName = 'paustianmelodymixermodule_score_' . ($isAdmin ? 'admin' : '') . 'view';
        ...
        $sortableColumns->addColumns([
            ...
            new Column('scoreOne'),
            new Column('scoreTwo'),
            new Column('scoreThree'),
            new Column('scoreFour'),
            new Column('scoreFive'),
            new Column('scoreSix'),
            new Column('scoreSeven'),
            new Column('scoreEight'),
            new Column('scoreNine'),
            new Column('scoreTen'),
     ...
     *
     * @throws AccessDeniedException Thrown if the user doesn't have required permissions
     * @throws NotFoundHttpException Thrown if score to be deleted isn't found
     * @throws RuntimeException Thrown if another critical error occurs (e.g. workflow actions not available)
     */
    protected function deleteInternal(
        ...
        if (!$permissionHelper->hasEntityPermission($score, $permLevel)) {
            throw new AccessDeniedException();
        }
        
        $logArgs = ['app' => 'PaustianMelodyMixerModule', 'user' => $currentUserApi->get('uname'), 'entity' => 'score', 'id' => $score->getKey()];
        
        // determine available workflow actions
        $actions = $workflowHelper->getActionsForObject($score);
        ...
        $redirectRoute = 'paustianmelodymixermodule_score_' . ($isAdmin ? 'admin' : '') . 'view';
        ...
        if (!$deleteAllowed) {
            $this->addFlash(
                'error',
                $this->trans(
                    'Error! It is not allowed to delete this score.',
                    [],
                    'score'
                )
            );
        ...
        $form = $this->createForm(DeletionType::class, $score);
        if ($score->supportsHookSubscribers()) {
            // call form aware display hooks
            $formHook = $hookHelper->callFormDisplayHooks($form, $score, FormAwareCategory::TYPE_DELETE);
        }
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            if ($form->get('delete')->isClicked()) {
                if ($score->supportsHookSubscribers()) {
                    ...
                    $validationErrors = $hookHelper->callValidationHooks($score, UiHooksCategory::TYPE_VALIDATE_DELETE);
                    if (0 < count($validationErrors)) {
                        ...
                        $success = $workflowHelper->executeAction($score, $deleteActionId);
                        if ($success) {
                            $this->addFlash(
                                'status',
                                $this->trans(
                        ...
                        if ($score->supportsHookSubscribers()) {
                            ...
                            $hookHelper->callFormProcessHooks($form, $score, FormAwareCategory::TYPE_PROCESS_DELETE);
                        
                            // Let any ui hooks know that we have deleted the score
                            $hookHelper->callProcessHooks($score, UiHooksCategory::TYPE_PROCESS_DELETE);
                        }
    ...
    protected function handleSelectedEntriesActionInternal(
        ...
        if (!is_array($items) || !count($items)) {
            return $this->redirectToRoute('paustianmelodymixermodule_score_' . ($isAdmin ? 'admin' : '') . 'view');
        }
