...
/**
 * Level controller base class.
 */
abstract class AbstractLevelController extends AbstractController
{
    ...
    protected function viewInternal(
        ...
        $routeName = 'paustianmelodymixermodule_level_' . ($isAdmin ? 'admin' : '') . 'view';
     ...
     *
     * @throws AccessDeniedException Thrown if the user doesn't have required permissions
     * @throws NotFoundHttpException Thrown if level to be deleted isn't found
     * @throws RuntimeException Thrown if another critical error occurs (e.g. workflow actions not available)
     */
    protected function deleteInternal(
        ...
        if (null === $level) {
            $level = $entityFactory->getRepository('level')->selectById($id);
        }
        if (null === $level) {
        ...
        if (!$permissionHelper->hasEntityPermission($level, $permLevel)) {
            throw new AccessDeniedException();
        }
        ...
        if (!$deleteAllowed) {
            $this->addFlash(
                'error',
                $this->trans(
                    'Error! It is not allowed to delete this level.',
                    [],
                    'level'
                )
            );
    ...
    protected function handleSelectedEntriesActionInternal(
        ...
        if (!is_array($items) || !count($items)) {
            return $this->redirectToRoute('paustianmelodymixermodule_level_' . ($isAdmin ? 'admin' : '') . 'view');
        }
