...
abstract class AbstractWorkflowEventsListener implements EventSubscriberInterface
{
                ...
                if (0 < count($entity->getGraphicsAndSound())) {
                    $event->addTransitionBlocker(
                        new TransitionBlocker(
                            $this->translator->__('Sorry, but you can not delete the level yet as it still contains graphics and sound!', [], 'level')
                        )
                    );
                }
                if (0 < count($entity->getMusicScores())) {
                    $event->addTransitionBlocker(
                        new TransitionBlocker(
                            $this->translator->__('Sorry, but you can not delete the level yet as it still contains music scores!', [], 'level')
                        )
                    );
                }
    ...
    protected function isEntityManagedByThisBundle($entity): bool
    {
        ...
        return 'Paustian' === $entityClassParts[0] && 'MelodyMixerModule' === $entityClassParts[1];
    }
}
