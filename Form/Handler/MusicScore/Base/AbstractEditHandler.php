...
namespace Paustian\MelodyMixerModule\Form\Handler\MusicScore\Base;

use Paustian\MelodyMixerModule\Form\Handler\Common\EditHandler;
use Paustian\MelodyMixerModule\Form\Type\MusicScoreType;
use Exception;
 ...
 * It aims on the music score object type.
 */
abstract class AbstractEditHandler extends EditHandler
{
    public function processForm(array $templateParameters = [])
    {
        $this->objectType = 'musicScore';
        $this->objectTypeCapital = 'MusicScore';
        $this->objectTypeLower = 'musicscore';
        ...
        if ('create' === $this->templateParameters['mode'] && !$this->modelHelper->canBeCreated($this->objectType)) {
            ...
            if ($request->hasSession() && ($session = $request->getSession())) {
                $session->getFlashBag()->add(
                    'error',
                    $this->trans(
                        'Sorry, but you can not create the music score yet as other items are required which must be created before!',
                        [],
                        'musicScore'
                    )
                );
    ...
    {
        return $this->formFactory->create(MusicScoreType::class, $this->entityRef, $this->getFormOptions());
    }
    ...
    protected function getRedirectCodes(): array
    {
        ...
        // user list of music scores
        $codes[] = 'userView';
        // admin list of music scores
        $codes[] = 'adminView';
        // user list of own music scores
        $codes[] = 'userOwnView';
        // admin list of own music scores
        $codes[] = 'adminOwnView';
    ...
    protected function getDefaultReturnUrl(array $args = []): string
    {
        ...
        // redirect to the list of musicScores
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ('create' === $this->templateParameters['mode']) {
                    $message = $this->trans('Done! Music score created.', [], 'musicScore');
                } else {
                    $message = $this->trans('Done! Music score updated.', [], 'musicScore');
                }
                break;
            case 'delete':
                $message = $this->trans('Done! Music score deleted.', [], 'musicScore');
                break;
            default:
                $message = $this->trans('Done! Music score updated.', [], 'musicScore');
                break;
