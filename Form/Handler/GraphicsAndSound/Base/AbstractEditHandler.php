...
namespace Paustian\MelodyMixerModule\Form\Handler\GraphicsAndSound\Base;

use Paustian\MelodyMixerModule\Form\Handler\Common\EditHandler;
use Paustian\MelodyMixerModule\Form\Type\GraphicsAndSoundType;
use Exception;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'graphicsAndSound';
        $this->objectTypeCapital = 'GraphicsAndSound';
        $this->objectTypeLower = 'graphicsandsound';
        ...
        if ('create' === $this->templateParameters['mode'] && !$this->modelHelper->canBeCreated($this->objectType)) {
            ...
            if ($request->hasSession() && ($session = $request->getSession())) {
                $session->getFlashBag()->add(
                    'error',
                    $this->trans(
                        'Sorry, but you can not create the graphics and sound yet as other items are required which must be created before!',
                        [],
                        'graphicsAndSound'
                    )
                );
    ...
    {
        return $this->formFactory->create(GraphicsAndSoundType::class, $this->entityRef, $this->getFormOptions());
    }
    ...
    protected function getRedirectCodes(): array
    {
        ...
        // user list of graphics and sound
        $codes[] = 'userView';
        // admin list of graphics and sound
        $codes[] = 'adminView';
        // user list of own graphics and sound
        $codes[] = 'userOwnView';
        // admin list of own graphics and sound
        $codes[] = 'adminOwnView';
    ...
    protected function getDefaultReturnUrl(array $args = []): string
    {
        ...
        // redirect to the list of graphicsAndSound
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ('create' === $this->templateParameters['mode']) {
                    $message = $this->trans('Done! Graphics and sound created.', [], 'graphicsAndSound');
                } else {
                    $message = $this->trans('Done! Graphics and sound updated.', [], 'graphicsAndSound');
                }
                break;
            case 'delete':
                $message = $this->trans('Done! Graphics and sound deleted.', [], 'graphicsAndSound');
                break;
            default:
                $message = $this->trans('Done! Graphics and sound updated.', [], 'graphicsAndSound');
                break;
    ...
    public function applyAction(array $args = []): bool
    {
        ...
        /** @var GraphicsAndSoundEntity $entity */
