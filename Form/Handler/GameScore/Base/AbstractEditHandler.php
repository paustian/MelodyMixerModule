...
namespace Paustian\MelodyMixerModule\Form\Handler\GameScore\Base;

use Paustian\MelodyMixerModule\Form\Handler\Common\EditHandler;
use Paustian\MelodyMixerModule\Form\Type\GameScoreType;
use Exception;
 ...
 * It aims on the game score object type.
 */
abstract class AbstractEditHandler extends EditHandler
{
    public function processForm(array $templateParameters = [])
    {
        $this->objectType = 'gameScore';
        $this->objectTypeCapital = 'GameScore';
        $this->objectTypeLower = 'gamescore';
        ...
        if ('create' === $this->templateParameters['mode'] && !$this->modelHelper->canBeCreated($this->objectType)) {
            ...
            if ($request->hasSession() && ($session = $request->getSession())) {
                $session->getFlashBag()->add(
                    'error',
                    $this->trans(
                        'Sorry, but you can not create the game score yet as other items are required which must be created before!',
                        [],
                        'gameScore'
                    )
                );
    ...
    {
        return $this->formFactory->create(GameScoreType::class, $this->entityRef, $this->getFormOptions());
    }
    ...
    protected function getRedirectCodes(): array
    {
        ...
        // user index page of game score area
        $codes[] = 'userIndex';
        // admin index page of game score area
        $codes[] = 'adminIndex';
        // user list of game scores
        $codes[] = 'userView';
        // admin list of game scores
        $codes[] = 'adminView';
        // user list of own game scores
        $codes[] = 'userOwnView';
        // admin list of own game scores
        $codes[] = 'adminOwnView';
    ...
    protected function getDefaultReturnUrl(array $args = []): string
    {
        ...
        // redirect to the list of gameScores
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ('create' === $this->templateParameters['mode']) {
                    $message = $this->trans('Done! Game score created.', [], 'gameScore');
                } else {
                    $message = $this->trans('Done! Game score updated.', [], 'gameScore');
                }
                break;
            case 'delete':
                $message = $this->trans('Done! Game score deleted.', [], 'gameScore');
                break;
            default:
                $message = $this->trans('Done! Game score updated.', [], 'gameScore');
                break;
